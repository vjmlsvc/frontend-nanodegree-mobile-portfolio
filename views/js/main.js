/*
 * Welcome to the 60fps project! Your goal is to make Cam's Pizzeria
 * website run jank-free at 60 frames per second.
 *
 * There are two major issues in this code that lead to sub-60fps
 * performance. Can you spot and fix both?
 *
 * Built into the code, you'll find a few instances of the User Timing
 * API (window.performance), which will be console.log()ing frame rate
 * data into the browser console. To learn more about User Timing API,
 * check out:
...
/* 
 * Generates random numbers for getAdj and getNoun functions and
 * returns a new pizza name
 */
...
/*
 * These functions return a string of a random ingredient from each
...
var resizePizzas = function(size) {
  ...
  /*
   * Changes the value for the size of the pizza above the slider; rewrote
   * the switch to use an anonymous function, reducing repetition
   */
  function changeSliderLabel(size) {
    document.querySelector("#pizzaSize").innerHTML = (function(){
    switch(size) {
      case "1":
        return "Small";
      case "2":
   ...
   * got rid of the needlessly complicated code for resizing and simply use a
...
/*
 * This for-loop actually creates and appends all of the pizzas when
 * the page loads
 */
...
/*
 * User Timing API again. These measurements tell you how long it took
 * to generate the initial pizzas
 */
...
/*
 * Iterator for number of times the pizzas in the background have
 * scrolled. Used by updatePositions() to decide when to log the
 * average time per frame
 */
var frame = 0;

/*
 * Logs the average amount of time per 10 frames needed to move the
 * sliding background pizzas on scroll
 */
...
/*
 * The following code for sliding background pizzas was pulled from
 * Ilya's demo found at:
 * https://www.igvita.com/slides/2012/devtools-tips-and-tricks/jank-demo.html
 *
 * Moves the sliding background pizzas based on scroll position
 *
 * Puts all .mover to array items[] and 5 different offsets to array phase[]
 * then iterates through everything in items[] offsetting them by one of the
 * offsets in phase[] depending on scroll distance position. Previous code
 * was generating the offset for each item every frame even though there were
 * only ever 5 possibilities. This code retains the same behaviour without
 * the huge impact on performance.
 */
function updatePositions() {
  ...
  /*
   * User Timing API to the rescue again. Seriously, it's worth
   * learning. Super easy to create custom metrics.
   */
